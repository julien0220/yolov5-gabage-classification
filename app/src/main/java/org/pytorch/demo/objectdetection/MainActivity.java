...
public class MainActivity extends AppCompatActivity implements Runnable {
    ...
    private String[] mTestImages = { "img_1.jpg", "img_2.jpg", "img_3.jpg" };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            // mModule = LiteModuleLoader
            // .load(MainActivity.assetFilePath(getApplicationContext(),
            // "yolov5s.torchscript.ptl"));
            // BufferedReader br = new BufferedReader(new
            // InputStreamReader(getAssets().open("classes.txt")));
            mModule = LiteModuleLoader
                    .load(MainActivity.assetFilePath(getApplicationContext(), "best.torchscript"));
            BufferedReader br = new BufferedReader(new InputStreamReader(getAssets().open("gabage.txt")));
            String line;
            ...
            switch (requestCode) {
                case 0:
                    if (resultCode == RESULT_OK && data != null) {
                        ...
                        mBitmap = Bitmap.createBitmap(mBitmap, 0, 0, mBitmap.getWidth(), mBitmap.getHeight(), matrix,
                                true);
                        mImageView.setImageBitmap(mBitmap);
                    }
                    break;
    ...
    public void run() {
        ...
        final Tensor inputTensor = TensorImageUtils.bitmapToFloat32Tensor(resizedBitmap, PrePostProcessor.NO_MEAN_RGB,
                PrePostProcessor.NO_STD_RGB);
        IValue[] outputTuple = mModule.forward(IValue.from(inputTensor)).toTuple();
        final Tensor outputTensor = outputTuple[0].toTensor();
        final float[] outputs = outputTensor.getDataAsFloatArray();
        final ArrayList<Result> results = PrePostProcessor.outputsToNMSPredictions(outputs, mImgScaleX, mImgScaleY,
                mIvScaleX, mIvScaleY, mStartX, mStartY);
